...
GROUP BY strftime('%Y-%m', v.voucher_date)
ORDER BY month;
```
...
JOIN vouchers v ON vd.voucher_id = v.id
JOIN account_subjects s ON vd.subject_id = s.id
WHERE vd.debit_amount > 100000 OR vd.credit_amount > 100000
